      ...
      */
      if (empty($mailuid) || empty($password)) {                                                        //check for empty inputs.
        ...
        exit();
            ...
            */
            $sql = "SELECT * FROM client WHERE username=? OR email=?;";                                                      //connect to database using prepared statements, sends SQl to database first, then fills in placeholders.
                ...
                if ($row = mysqli_fetch_assoc($result)) {                                               //store results into variable.
                    ...
                    if ($pwdCheck == false) {                                                           //if they don't match, error message.
                      ...
                      exit();
                        ...
                        */
                        session_start();
                        $_SESSION['cid'] = $row['clientID'];                                            //session clientID variable.
                        
                        $_SESSION['uid'] = $row['username'];                                            //session username variable.
                        $_SESSION['email'] = $row['email'];                                             //session email variable.
                        $_SESSION['gender'] = $row['gender'];                                           //session gender variable.
                        $_SESSION['phone'] = $row['clientPhoneNumb'];                                   //session phone number variable.
                        $_SESSION['country'] = $row['country'];                                         //session country variable.
                        $_SESSION['health'] = $row['healthProblem'];                                    //session health problem variable.
                        $_SESSION['height'] = $row['height'];                                           //session height variable.
                        $_SESSION['dateOfBirth'] = $row['dateOfBirth'];                                 //session date of birth variable.
