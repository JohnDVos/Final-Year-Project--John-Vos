      ...
      */
      if (empty($mailuid) || empty($password)) {                                                        //check for empty inputs.
        ...
        exit();
            ...
            */
            $sql = "SELECT * FROM users WHERE uidUsers=? OR emailUsers=?;";                             //connect to database using prepared statements, sends SQl to database first, then fills in placeholders.
            ...
            if (!mysqli_stmt_prepare($stmt, $sql)) {                                                    //prepares SQL statement & check for any errors.
                ...
                if ($row = mysqli_fetch_assoc($result)) {                                               //store results into variable.
                    $pwdCheck = password_verify($password, $row['pwdUsers']);                           //match password from database with password from user, result returned as boolean.
                    if ($pwdCheck == false) {                                                           //if they don't match, error message.
                      ...
                      exit();
                        ...
                        */
                        session_start();
                        $_SESSION['id'] = $row['idUsers'];                                              //session variable.
                        $_SESSION['uid'] = $row['uidUsers'];                                            //session variable.
                        $_SESSION['email'] = $row['emailUsers'];                                        //session variable.
